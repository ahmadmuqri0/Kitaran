...
function handleLogin(event) {
    ...
    if(user) {
        ...
        window.location.href = (user.toLowerCase() === 'admin') ? 'admin.html' : 'dashboard.html';
    }
}
...
function loadAdminPanel(filter = 'all') {
    ...
    const filteredData = history.filter(entry => {
        ...
        if (filter === 'verified') return entry.status === 'Verified' && entry.penalty == 0;
        if (filter === 'penalty') return entry.penalty > 0;
        return true; 
    });
    ...
    filteredData.forEach((entry) => {
        if(entry.status === 'Verified') totalSystemWeight += parseFloat(entry.weight);
...
function confirmPenalty() {
    ...
    if(!wrongWeight || wrongWeight <= 0) return alert("Please fill in the miss-categorized weight");
...
function loadUserDashboard() {
    ...
    myData.forEach(entry => {
        if(entry.weight > 0) totalMyWeight += parseFloat(entry.weight);
